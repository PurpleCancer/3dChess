...
void writeH(string fp, string name, Model model)
{
    ...
    // Write declarations
    outH << "int " << name << "Vertices;" << endl;
    outH << "float " << name << "Positions[" << model.vertices*3 << "];" << endl;
    outH << "float " << name << "Texels[" << model.vertices*2 << "];" << endl;
    outH << "float " << name << "Normals[" << model.vertices*3 << "];" << endl;
    outH << endl;
...
void writeCvertices(string fp, string name, Model model)
{
    ...
    // Vertices
    outC << "int " << name << "VertexCount = " << model.vertices << ";" << endl;
    outC << endl;
...
void writeCpositions(string fp, string name, Model model, int faces[][9], float positions[][3])
{
    ...
    // Positions
    outC << "float " << name << "Vertices[" << model.vertices*3 << "] = " << endl;
    outC << "{" << endl;
...
void writeCtexels(string fp, string name, Model model, int faces[][9], float texels[][2])
{
    ...
    // Texels
    outC << "float " << name << "TexCoords[" << model.vertices*2 << "] = " << endl;
    outC << "{" << endl;
    for(int i=0; i<model.faces; i++)
    {
        int vtA = faces[i][1] - 1;
...
void writeCnormals(string fp, string name, Model model, int faces[][9], float normals[][3])
{
    ...
    // Normals
    outC << "float " << name << "Normals[" << model.vertices*3 << "] = " << endl;
    outC << "{" << endl;
    for(int i=0; i<model.faces; i++)
    {
        int vnA = faces[i][2] - 1;
...
void write2file(string fp,string ofp)
{
    
    // Open OBJ file
    ifstream inOBJ;
    inOBJ.open(fp,ios::in | ios::binary);
    if(!inOBJ.good())
    {
        cout << "ERROR OPENING OBJ FILE" << endl;
        exit(1);
...
char * buffer = new char [length];
    ...
    inOBJ.read (buffer,length);
outC.write(buffer,length);
*/
...
string filepathVBO = nameOBJ + ".vbo";
string filepathTXT = nameOBJ + ".txt";

//write2file(filepathVBO,filepathTXT);
